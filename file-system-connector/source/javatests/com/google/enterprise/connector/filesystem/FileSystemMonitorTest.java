...
public class FileSystemMonitorTest extends TestCase {
	...
	private class CountingVisitor implements FileSystemMonitor.Callback {
		...
		private boolean keepAddingFiles = false; // Ensures modification every
													// pass.
		...
		/*
		 * Giving a monitor reference makes this vistior update it's monitor
		 * with checkpoints at passComplete.
		 */
		...
		public void passComplete(MonitorCheckpoint mcp)
			...
			++scanCount;
			if (myMonitor != null) {
				myMonitor.acceptGuarantee(mcp); // On maxScans pass let's
												// monitor know
												// it can drop snapshots <
												// (maxScans - 1).
												// deleteOldSnapshots() needs to
												// be called
												// for this pruning to actually
												// happen.
			}
			if (scanCount >= maxScans) {
				// Last snapshot (maxScans) is expected written and closed now.
				// So if maxScans was 10 there is a closed file called snap.10
				// in
				// snapshots directory. Next call to deleteOldSnapshots() would
				// delete
	...
	/**
	 * A {@link FileSystemMonitor.Callback} for running a scan, making a file
	 * system change and running another scan. The caller should override the
	 * call back for her change and throw an {@link InterruptedException} for
	 * the specific change. On the initial scan passComplete will remain false.
	 */
	...
	/**
	 * Adds a file after a complete pass and throws an
	 * {@link InterruptedException} in
	 * {@link #newFile(FileInfo, MonitorCheckpoint)} for the added file on the
	 * next pass.
	 */
	private class StopAfterSpecificAddVisitor extends
			ChangeOnPassCompleteVisitor {
	 ...
	 * next pass.
	 */
	private class StopAfterReplaceFileVisitor extends
			ChangeOnPassCompleteVisitor {
	...
	private FileSystemMonitor newFileSystemMonitor(FilePatternMatcher myMatcher) {
		return newFileSystemMonitor(checksumGenerator, myMatcher, new FakeTraversalContext(), new LoggingFileSink());
	}

	private FileSystemMonitor newFileSystemMonitor(ChecksumGenerator generator) {
		return newFileSystemMonitor(generator, matcher, new FakeTraversalContext(), new LoggingFileSink());
	}

	private FileSystemMonitor newFileSystemMonitor(
			TraversalContext traversalContext) {
		return newFileSystemMonitor(checksumGenerator, matcher, traversalContext, new LoggingFileSink());
	}
	...
	private FileSystemMonitor newFileSystemMonitor(
		...
		return new FileSystemMonitor("name", root, store, customVisitor,
				checksumGenerator, matcher, new FakeTraversalContext(),
				new LoggingFileSink(), null, null);
	}

	@Override
	public void setUp() throws Exception {
		TestDirectoryManager testDirectoryManager = new TestDirectoryManager(
				this);
	 ...
	 * @throws IOException
	 */
	private void createTree(MockReadonlyFile folder, int width, int depth,
	...
	private void runMonitorAndCheck(int newDirs, int newFiles, int deletedDirs,
			int deletedFiles, int contentChanges, int fileMetadataChanges,
			int dirMetadataChanges) {
		monitor.run();
	...
	public void testSomeFilesTooBig() throws Exception {
		...
		monitor = newFileSystemMonitor(new FakeTraversalContext(
				BIG_CONTENT.length() - 1), sink);
		runMonitorAndCheck(baseDirs, baseFiles, 0, 0, 0, 0, 0);
		visitor.passBegin();
	...
	public void testAddNotSupportedMimeType() {
		...
		root.addFile("unsupported." + FakeTraversalContext.TAR_DOT_GZ_EXTENSION, "not a real zip file");
		runMonitorAndCheck(baseDirs, baseFiles, 0, 0, 0, 0, 0);
	...
	public void testSupportedToNotSupportedMimeType() {
		...
		MockReadonlyFile unsupportedAfterUpdate = root.addFile("unsupportedAfterUpdate."
				+ FakeTraversalContext.TAR_DOT_GZ_EXTENSION, "not a real zip file");
		runMonitorAndCheck(baseDirs, baseFiles + 1, 0, 0, 0, 0, 0);
	...
	public void testNotSupportedToSupportedMimeType() {
		FakeTraversalContext traversalContext = new FakeTraversalContext();
		monitor = newFileSystemMonitor(traversalContext);
		MockReadonlyFile supportedAfterUpdate = root.addFile("unsupportedAfterUpdate."
				+ FakeTraversalContext.TAR_DOT_GZ_EXTENSION, "not a real zip file");
		runMonitorAndCheck(baseDirs, baseFiles, 0, 0, 0, 0, 0);
	...
	public void testFileDeletion() {
		...
		String[] newFiles = new String[] { "aaa-new-file", "dir.1.new-file",
				"eee-new-file", "file.1.zzz-file", "zzz.new-file" };
	...
	public void testChangeAclNotStable() throws Exception {
		...
		// The second scan should checksum all files including the 2 with
		// changed
		// ACLs.
		Acl acl2 = Acl.newAcl(Arrays.asList("fred", "jill"), Arrays.asList("admins"));
		file1.setAcl(acl2);
		file2.setAcl(acl2);
		clock.advance(10000);
		...
		visitor.passBegin();
		// The third scan should checksum the 2 files that are not yet stable
		// but
	...
	public void testChangeChangeAclStable() throws Exception {
		...
		visitor.passBegin();
		// The third scan reflects all the files stable so no changes or
		// checksums
		// should be detected.
		clock.advance(10000);
		runMonitorAndCheck(0, 0, 0, 0, 0, 0, 0);
	...
	public void testPatternBasics() {
		FilePatternMatcher nonPngMatcher = FileConnectorType.newFilePatternMatcher(ImmutableList.of("/", "", "# comment"), ImmutableList.of(".png$", "\t", "# another comment"));
	...
	public void testChangePatterns() {
		...
		FilePatternMatcher nonPngMatcher = FileConnectorType.newFilePatternMatcher(ImmutableList.of("/"), ImmutableList.of(".png$"));
		monitor = newFileSystemMonitor(nonPngMatcher);
	...
	public void testSnapshotRestoreOnNewFile() throws Exception {
		root.addSubdir("fila.brazila"); // Testing to see this dir in stiched
										// snapshot.
		StopAfterSpecificAddVisitor stoppingVisitor = new StopAfterSpecificAddVisitor(
				"fila.b"); // Sorts before fila.brazila
	...
	public void testSnapshotRestoreAfterReplaceFile() throws Exception {
		StopAfterReplaceFileVisitor replaceVisitor = new StopAfterReplaceFileVisitor(
				"file.6.txt");
	...
	private class FailingSnapshotStore extends SnapshotStore {
		...
		@Override
		public SnapshotWriter openNewSnapshotWriter()
				throws SnapshotStoreException {
			boolean threw = writer != null && writer.threw;
			writer = new FailingSnapshotWriter(threw,
					super.openNewSnapshotWriter(), interrupt);
			return writer;
		}
	}
	...
	private ArrayList<String> readEntireMostRecentSnapshot(SnapshotStore store) {
		try {
			...
			for (SnapshotRecord record = reader.read(); record != null; record = reader.read()) {
				records.add(record.getPath()); // Use paths cause scan times are
												// different.
			}
			return records;
		} catch (Exception e) {
			throw new IllegalStateException("Reading snapshot in test failed.",
					e);
		}
	}
	...
	public void testSevereExceptionCausesReStich() throws Exception {
		...
		/* Cause failure. */
		FailingSnapshotStore failingStore = new FailingSnapshotStore(
				saveOldestSnapshotToKeep, false);

		visitor = new CountingVisitor();
		monitor = new FileSystemMonitor("name", root, failingStore, visitor,
				checksumGenerator, matcher, new FakeTraversalContext(),
	...
	public void testInterruptExceptionRecovery() throws Exception {
		 ...
		 * FileSystemMonitor recovery to exit and the FileSystemMonitor.run() to
		 * exit.
		 */
		FailingSnapshotStore failingStore = new FailingSnapshotStore(
				saveOldestSnapshotToKeep, true);
		...
		// starting a new pass because
		// FileSystemMonitor.performExceptionRecovery
