 ...
 * <p>
 * This implementation works as follows. It repeatedly scans all the files and
 ...
 * <p>
 * A note about deciding whether a file has changed: One way to do this is to
 ...
 * not.
 * <p>
 * So here is the theory of operation: suppose we checksum a file, then some
...
public class FileSystemMonitor implements Runnable {
	...
	private List<String> excludedDirectoriesFromScan = new ArrayList<String>();
	/**
	 * Minimum interval in ms between identical checksums for a file to be
	 * deemed "stable". This should be at least several times the granularity of
	 ...
	 */
	public FileSystemMonitor(String name, ReadonlyFile<?> root,
			SnapshotStore snapshotStore, Callback callback,
			ChecksumGenerator checksumGenerator, FilePatternMatcher matcher,
			TraversalContext traversalContext, FileSink fileSink,
			MonitorCheckpoint initialCp, List<String> excludedList) {
		this.name = name;
		this.root = root;
		...
		this.excludedDirectoriesFromScan = excludedList;
	...
	public void run() {
		try {
			...
			LOG.log(Level.INFO, "FileSystemMonitor " + name
	...
	private void tryToRunForever() throws InterruptedException {
		try {
			...
			String msg = "Failed to write to snapshot file: "
					+ snapshotWriter.getPath();
			LOG.log(Level.SEVERE, msg, e);
		} catch (SnapshotReaderException e) {
			String msg = "Failed to read snapshot file: "
	...
	/**
	 * Call in situations were FileSystemMonitor runs were interfered with and
	 * we wish to have the FileSystemMonitor continue running. Brings system
	 * into state where doOnePass can be invoked. Failures in this method are
	 * considered fatal for the thread.
	 * 
	...
	private void performExceptionRecovery() throws InterruptedException,
		...
		try {
			...
			LOG.info("FileSystemMonitor " + name
					+ " closed faulty reader and writer.");
		} catch (IOException e) {
		...
		if (null == guaranteeCheckpoint) {
			...
			String msg = "FileSystemMonitor " + name
	...
	private void doOnePass() throws SnapshotStoreException,
		...
		if (!callback.hasEnqueuedAtLeastOneChangeThisPass()) {
			...
			// there were no changes, so we can delete the snapshot we just
			// wrote.
	 ...
	 * @throws InterruptedException
	 */
	private void processFileOrDir(ReadonlyFile<?> fileOrDir)
			throws SnapshotReaderException, SnapshotWriterException,
			InterruptedException, DirListingFailedException {
	 ...
	 * @throws InterruptedException
	 */
	private void processDeletes(ReadonlyFile<?> file)
			throws SnapshotReaderException, InterruptedException {
		while (current != null
				&& (file == null || pathCompare(file, current) > 0)) {
	...
	private void safelyProcessFile(ReadonlyFile<?> file)
	...
	private void processFile(ReadonlyFile<?> file) throws InterruptedException,
		...
		if ((traversalContext != null)
				&& (traversalContext.maxDocumentSize() < file.length())) {
		...
		if (current != null && (pathCompare(current, file) == 0)) {
			...
			writeFileSnapshot(file.getFileSystemType(), file.getPath(), file.getLastModified(), infoCache.getAcl(), infoCache.getChecksum(), false);
	 ...
	 * @throws SnapshotReaderException
	 */
	private void processPossibleFileChange(ReadonlyFile<?> file)
			throws IOException, InterruptedException, SnapshotWriterException,
		...
		if (current.getFileType() == SnapshotRecord.Type.DIR) {
			...
			callback.newFile(file, getCheckpoint());
				...
				|| (!current.isStable() && !current.getChecksum().equals(infoCache.getChecksum()))) {
			writeFileSnapshot(file.getFileSystemType(), file.getPath(), file.getLastModified(), infoCache.getAcl(), infoCache.getChecksum(), false);
			...
			writeFileSnapshot(current.getFileSystemType(), current.getPath(), current.getLastModified(), current.getAcl(), current.getChecksum(), isStable);
		}
		current = snapshotReader.read();
	}

	void writeDirectorySnapshot(ReadonlyFile<?> file) throws IOException,
			SnapshotWriterException {
		if (!file.isDirectory()) {
			throw new IllegalArgumentException("File is not a directory: "
					+ file);
		}
		SnapshotRecord rec = new SnapshotRecord(file.getFileSystemType(),
				file.getPath(), SnapshotRecord.Type.DIR,
				file.getLastModified(), file.getAcl(), "", clock.getTime(),
				false);
		snapshotWriter.write(rec);
	}

	void writeFileSnapshot(String fileSystemType, String path,
			long lastModified, Acl acl, String checksum, boolean stable)
			throws SnapshotWriterException {
		SnapshotRecord rec = new SnapshotRecord(fileSystemType, path,
				SnapshotRecord.Type.FILE, lastModified, acl, checksum,
				clock.getTime(), stable);
		snapshotWriter.write(rec);
	}
	...
	private void processDirectory(ReadonlyFile<?> dir)
		...
		if (current != null && (pathCompare(current, dir) == 0)) {
			if (current.getFileType() == SnapshotRecord.Type.FILE) {
				...
				try {
					...
					LOG.log(Level.INFO, "Failed getting info off directory.", e);
					excludedDirectoriesFromScan.add(dir.getPath());
				}
			}
			current = snapshotReader.read();
		} else {
			...
			try {
				// checking exclidedDirectoriesList before writing it to
				// snapshot.
				if (!excludedDirectoriesFromScan.contains(dir.getPath())) {
					writeDirectorySnapshot(dir);
				}
			} catch (IOException e) {
				LOG.log(Level.INFO, "Failed getting info off directory.", e);
		...
		try {
			...
			// During initial scan log the exception and add it to
	 ...
	 * <p>
	 * Determining a file's mime type can be an expensive operation and require
	 * reading the file. To avoid computing the mime type for every file on
	 * every pass we add files with unsupported mime types to the snapshot. This
	 * allows us to cheaply ignore the file on future passes based on an
	 ...
	 * to the GSA. This would enhance the usefulness of the snapshot as a recode
	 * of the content of the GSA.
	 * <li>Store the mime type in the {@link Change} to avoid recomputing the
	 * value in {@link FileFetcher}. This requires detecting modifications to
	 * the file between the time the mime type is added to the change and the
	 * time the file is sent to the GSA. There is already a race related to the
	 * fact computing the mime type and reading the file are not atomic.
	 * </ol>
	 */
	private class FilteringCallback implements Callback {
		...
		private boolean isMimeTypeSupported(FileInfo f) {
			...
			try {
				String mimeType = mimeTypeFinder.find(traversalContext, f.getPath(), new FileInfoInputStreamFactory(
						f));
				...
				// Note the GSA will filter files with unsuported mime types so
				// by
				// sending the file we may expend computer resources but will
				// avoid
